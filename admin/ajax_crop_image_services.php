<?php
	...
	if($_POST['type'] ==1)
	{
		...
		if($success != false)
		{
			...
			VALUES ($page_id,$org_id,$category_id,'".$title."','".$data."','".$imageName."',1,0)";
	...
	if($_POST['type'] ==2)
	{
		...
		if($success != false)
		{
			$sql = "UPDATE `pag_dtl01` SET `PAD_PACD`='".$page_id."',`PAD_ORCD`='" . $org_id . "',`PAD_CACD`=$category_id,`PAD_TITL`='" . $title . "',`PAD_DESC`='" . $data . "',`PAD_IMG`='". $imageName."',`PAD_API`='',`PAD_ADSO`='',`PAD_ADST`='',`PAD_MAIL`='',`PAD_PHN`='',`PAD_STAT`=1,`PAD_CANC`=0 WHERE PAD_PGCD = $edit_id ";
			$result =mysqli_query($conn, $sql);
