<?php
    ...
    $address= mysqli_real_escape_string($conn,$_POST['dip_order']);

	$output_dir = "uploadimage/galleryimg/";
	...
	if($_POST['type'] ==1)
	{
		...
		if($success != false)
		{
			$sql="INSERT INTO `picture_gal`(`orcd`,`name`,`address`, `description`, `image`,`status`) VALUES ($org_id,'$title','$address','$data','$imageName',1)";
	...
	if($_POST['type'] ==2)
	{
		...
		if($success != false)
		{
			$sql = "UPDATE `picture_gal` SET `name`='" . $title . "',`description`='" . $data . "',`image`='". $imageName."' WHERE category_id = $edit_id ";
			$result =mysqli_query($conn, $sql);
