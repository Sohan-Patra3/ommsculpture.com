<?php
	...
	if($_POST['type'] ==1)
	{
		...
		if($success != false)
		{
			$sql="INSERT INTO `category_mst`(`orcd`,`name`,  `description`, `image`,`status`) VALUES ($org_id,'$title','$data','$imageName',1)";
	...
	if($_POST['type'] ==2)
	{
		...
		if($success != false)
		{
			$sql = "UPDATE `category_mst` SET `name`='" . $title . "',`description`='" . $data . "',`image`='". $imageName."' WHERE category_id = $edit_id ";
			$result =mysqli_query($conn, $sql);
