...
if(isset($_POST['submit'])){
	if($edit_id ==''){
		...
		if (mysqli_query($conn, $sql)) {
			...
			header("Location: latestnews.php?page_id=$page_id&status=200");
		...
		if (mysqli_query($conn, $sql)) {
			...
			header("Location: latestnews.php?page_id=$page_id&status=201");
		}else {
...
if(isset($_GET['delitid'])){
	...
	if(mysqli_query($conn,$sql)){
		header("Location: latestnews.php?page_id=$page_id&status=203");
	}else{
		header("Location: latestnews.php?page_id=$page_id&status=300");
	}
}
